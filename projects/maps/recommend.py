...
def find_closest(location, centroids):
    ...
    for i in range(1, len(centroids)):
        if min_dis > distance(location, centroids[i]):
            min_dis = distance(location, centroids[i])
            flag = i
...
def group_by_centroid(restaurants, centroids):
    ...
    for rest in restaurants:
        cen = find_closest(restaurant_location(rest), centroids)
        cloest.append([cen, rest])
    return group_by_first(cloest)
    # END Question 4
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    ave_lat = mean([restaurant_location(clu)[0] for clu in cluster])
    ave_lon = mean([restaurant_location(clu)[1] for clu in cluster])
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(clu) for clu in cluster]
        # END Question 6
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    for rest in restaurants:
        if rest in reviewed:
            dict_rate[restaurant_name(rest)] = user_rating(user, restaurant_name(rest))
        else:
            dict_rate[restaurant_name(rest)] = predictor(rest)
    return dict_rate
    # END Question 9
