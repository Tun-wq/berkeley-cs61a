...
def balanced(m):
    ...
    #     if size(end(left(m))) * length(left(m)) == total_weight(end(right(m))) * length(right(m)):
    #         return balanced(end(right(m)))
    #     else:
    #         return False
    # elif is_mobile(end(left(m))) and is_weight(end(right(m))):
    #     if size(end(right(m))) * length(right(m)) == total_weight(end(left(m))) * length(left(m)):
    #         return balanced(end(left(m)))
    #     else:
    #         return False
    # else:
    #     if size(end(left(m))) * length(left(m)) == size(end(right(m))) * length(right(m)):
    #         return True
    #     else:
    #         return False
    if is_weight(m):
        return True
    l, r = left(m), right(m)
    ba = length(l) * total_weight(end(l)) == length(r) * total_weight(end(r))
    return ba and balanced(end(l)) and balanced(end(r))
...
def make_withdraw(balance, password):
    ...
    error_times = 0
    error_pwd = []
    def password_protected_withdraw(amount, input_pwd):
        nonlocal balance, password, error_times, error_pwd
        if amount <= balance and input_pwd == password and error_times < 3:
            balance = balance - amount
            return balance
        elif input_pwd != password or error_times >= 3:
            error_times += 1
            if error_times <= 3:
                error_pwd.append(input_pwd)
                return 'Incorrect password'
            else:
                return "Your account is locked. Attempts: " + str(error_pwd)
        elif amount > balance:
